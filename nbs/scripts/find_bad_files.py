...
try:
    ...
    def check_uncertainty(pth):
        try:
            ds = gdal.Open(pth)
        except (RuntimeError, FileNotFoundError):
            print("failed to open", pth)
        else:
            bnd = ds.GetRasterBand(2)
            # have to check full data since GetStats(True, False) can return 0,0 even when data exists (Fuse csar conversion is not setting stats)
            stats = bnd.GetStatistics(True, True)
            del bnd
            del ds
            if stats[:2] == [0.0, 0.0]:
                print(stats, time.ctime(pathlib.Path(pth).stat().st_mtime), pth)
                os.remove(pth)
            else:
                print("ok", stats, pth)
...
for data_type in ("NBS_Data_Unqualified", "NBS_Data_Sensitive", "NBS_Data_Qualified"):
    ...
    for full_path in glob.glob(fr"\\nos.noaa\OCS\HSD\Projects\NBS\{data_type}\**\*.csar.tif", recursive=True):
        if is_caris:
