...
from nbs.bruty.nbs_no_locks import LockNotAcquired, AreaLock, FileLock, EXCLUSIVE, SHARED, NON_BLOCKING, SqlLock, NameLock, current_address
"""
#----------------------------------------------------
...
engine = sqlalchemy.create_engine('postgresql+psycopg2://{username}:{password}@{hostname}:{port}/metadata')
Base = automap_base()
Base.prepare(engine, reflect=True)
...
def use_locks(port):
    global LockNotAcquired, AreaLock, FileLock, EXCLUSIVE, SHARED, NON_BLOCKING, SqlLock, NameLock, NO_LOCK, current_address
    if port is None:
        from nbs.bruty.nbs_no_locks import LockNotAcquired, AreaLock, FileLock, EXCLUSIVE, SHARED, NON_BLOCKING, SqlLock, NameLock, current_address
        NO_LOCK = True
    else:
...
class SqliteSurveyMetadata(MutableMapping):
    ...
    def to_python(self, rec):
        return [sql_data_field.to_python(v) for v, sql_data_field in zip(rec, self.fields)]
    ...
    def get_records(self, field, val):
        if self._uses_oid:
            ...
            proc_records = [[record[0]] + self.to_python(record[1:]) for record in raw_records]
        else:
    ...
    def __setitem__(self, key, val):
        ...
        with SqlLock(self.conn):
            if not self._uses_oid:
                record = self.to_sql([*val[:self._primary_index], key, *val[self._primary_index:]])
            else:
                record = self.to_sql(val)
            if not self._uses_oid:
                # @todo - see if there is a significant performance penalty to doing update and insert if that fails, if not then switch to always doing that
...
class IncludedMetadata(SqliteSurveyMetadata):
    ...
    def create_table(self):
        ...
        createSecondaryIndex = f"CREATE INDEX index_{self.tablename}_survey_path ON {self.tablename}(survey_path)"
...
# @todo change the sqlite records classes to allow names to be used like .tiles
...
class WorldTilesBackend(VABC):
                ...
                for ty_dir in tqdm(list(os.scandir(tx_dir)), desc='Y directories', mininterval=.7, leave=False):
                    if ty_dir.is_dir():
                        try:
                            ...
                            print(f"non-tile directory in the database {tx}, {ty}")
                        else:
    ...
    def get_tile_history_by_index(self, tx, ty, must_exist=False):
        hist_path = self.get_history_path_by_index(tx, ty)
        if must_exist and not self.storage_class.exists(hist_path):
    ...
    def iter_tiles(self, x, y, x2, y2, must_exist=False):
        for tx, ty in self.get_tiles_indices(x, y, x2, y2):
            history = self.get_tile_history_by_index(tx, ty, must_exist=must_exist)
...
class WorldDatabase(VABC):
        ...
        for sid, val in tqdm(self.included_ids.items(), total=len(self.included_ids), desc='Surveys', mininterval=.7, leave=False):
            ...
            if len(tiles) > 1:
                ...
                if max_y > tolerance or max_x > tolerance:
                    possible_errors[sid] = (max_x, max_y, val[0], val[1])
            elif len(tiles) == 0:
    ...
    def validate(self):
        ...
        for contrib in tqdm(self.included_ids.keys(), desc='Surveys', mininterval=.7, leave=False):
            for tx, ty in self.included_ids[contrib][1]:
    ...
    def insert_survey(self, path_to_survey_data, override_epsg=NO_OVERRIDE, contrib_id=numpy.nan, compare_callback=None, reverse_z=False,
                      limit_to_tiles=None, force=False, survey_score=100, flag=0, dformat=None, transaction_id=-1):
        """
        Parameters
        ----------
        path_to_survey_data
        ...
        if extension == '.bag':
            try:
                ...
                self.insert_survey_vr(vr, override_epsg=override_epsg, contrib_id=contrib_id, compare_callback=compare_callback, reverse_z=reverse_z,
                                      limit_to_tiles=limit_to_tiles, force=force, survey_score=survey_score, flag=flag, transaction_id=transaction_id)
                done = True
        if not done:
        ...
        if not done:
            if extension in ['.txt', '.npy', '.csv', '.npz']:
                self.insert_txt_survey(path_to_survey_data, override_epsg=override_epsg, contrib_id=contrib_id, compare_callback=compare_callback,
                                       reverse_z=reverse_z, limit_to_tiles=limit_to_tiles, force=force, survey_score=survey_score, flag=flag,
                                       dformat=dformat, transaction_id=transaction_id)
                done = True
        if not done:
    ...
    def _get_transformer(self, srs, override_epsg, path_to_survey_data):
        try:
    ...
    def insert_survey_gpkg(self, path_to_survey_data, survey_score=100, flag=0, override_epsg=NO_OVERRIDE,
                           contrib_id=numpy.nan, compare_callback=None, reverse_z=False, limit_to_tiles=None, force=False, transaction_id=-1):
        ...
        for ilyr in range(gpkg.GetLayerCount()):
            ...
            if lyr.GetGeomType() == ogr.wkbPoint:
                ...
                self._insert_xyz(x, y, depth, uncertainty, survey_score, flag, contrib_id, path_to_survey_data, compare_callback,
                                 override_epsg, reverse_z, limit_to_tiles=limit_to_tiles, force=force, dformat="gpkg", transaction_id=transaction_id)
            ...
            with tqdm(desc="geocode/process points", total=2) as top_progress_bar:
                with tqdm(desc="geocode+split", total=0, leave=False) as progress_bar:
                    ...
                    with AreaLock(tile_list, EXCLUSIVE | NON_BLOCKING, self.db.get_history_path_by_index) as lock:
                        ...
                        self.finished_survey_insertion(path_to_survey_data, tile_list, contrib_id, override_epsg, reverse_z, survey_score, flag, dformat,
                                                       transaction_id)
    ...
    def _insert_xyz(self, x, y, depth, uncertainty, survey_score, flag, contrib_id, path_to_survey_data, compare_callback, override_epsg, reverse,
                    limit_to_tiles=None, force=False, dformat=None, transaction_id=-1):
    ...
    def finished_survey_insertion(self, path_to_survey_data, tiles, contrib_id=numpy.nan,
                                  override_epsg=NO_OVERRIDE, reverse_z=False, survey_score=100, flag=0, dformat=None, transaction_id=-1):
        ...
        with FileLock(self.metadata_filename(), self.metadata_mode(), EXCLUSIVE) as metadata_file:
            ...
            if str(path_to_survey_data) not in self.included_surveys:
                self.included_surveys[str(path_to_survey_data)] = (
                contrib_id, [(tx, ty) for tx, ty in tiles], override_epsg, reverse_z, survey_score, flag, dformat, mtime,
                transaction_id)  # json doesn't like sets, convert to list
                if contrib_id is not None:
                    self.included_ids[contrib_id] = (
                    str(path_to_survey_data), [(tx, ty) for tx, ty in tiles], override_epsg, reverse_z, survey_score, flag, dformat, mtime,
                    transaction_id)
    ...
    def start_survey_insertion(self, path_to_survey_data, tiles, contrib_id=numpy.nan, transaction_id=-1):
        ...
        with FileLock(self.metadata_filename(), self.metadata_mode(), EXCLUSIVE) as metadata_file:
            ...
            if str(path_to_survey_data) in self.started_surveys:
                existing_mtime = self.started_surveys[path_to_survey_data][2]
            elif contrib_id in self.started_ids:
                existing_mtime = self.started_ids[contrib_id][2]
            else:
            ...
            if str(path_to_survey_data) not in self.started_surveys:
                self.started_surveys[str(path_to_survey_data)] = (
                contrib_id, [(tx, ty) for tx, ty in tiles], mtime, transaction_id)  # json doesn't like sets, convert to list
                if contrib_id is not None:
                    self.started_ids[contrib_id] = (str(path_to_survey_data), [(tx, ty) for tx, ty in tiles], mtime, transaction_id)
            # Sqlite file updates immediately, so no longer need to call to_file() which only stores the class setup.
            # # rather than overwrite, since we have it locked, truncate the file and then write new data to it
            # metadata_file.seek(0)
            # metadata_file.truncate(0)
    ...
    def insert_survey_vr(self, vr, survey_score=100, flag=0, override_epsg=NO_OVERRIDE, contrib_id=numpy.nan, compare_callback=None, reverse_z=False,
                         limit_to_tiles=None, force=False, transaction_id=-1):
        """
        Parameters
        ----------
        vr
        ...
        if crs_transformer is not None:
            ((x1, x2), (y1, y2)) = crs_transformer.transform((x1, x2), (y1, y2))
        ...
        with AreaLock(tile_list, EXCLUSIVE | NON_BLOCKING, self.db.get_history_path_by_index) as lock:
            if force or contrib_id is None or contrib_id not in self.included_ids:
                ...
                self.finished_survey_insertion(path_to_survey_data, all_tiles, contrib_id, override_epsg, reverse_z, survey_score, flag,
                                               dformat="gdal", transaction_id=transaction_id)
            else:
                raise Exception(f"Survey Exists already in database {contrib_id}")
    ...
    def export_into_raster(self, dataset, dataset_score, target_epsg=None,
        ...
        # for txi, tyi, tile_history in self.db.iter_tiles(overview_x1, overview_y1, overview_x2, overview_y2, must_exist=True):
        for txi, tyi, tile_history in tqdm(self.db.iter_tiles(overview_x1, overview_y1, overview_x2, overview_y2, must_exist=True),
    ...
    def clean(self, invalid_score_surveys, unfinished_surveys=(), out_of_sync=(), compare_callback=None, transaction_id=-1, subprocesses=5):
        """
        Parameters
        ----------
    ...
    def remove_survey(self, contributor, transaction_id=-1):
        ...
        # 1) Find all the tiles the ID falls in and make a master list of tile numbers (this is in master metadata record?)
        path_to_file, tile_list = self.started_ids[contributor][:2]
        ...
        with AreaLock(tile_list, EXCLUSIVE | NON_BLOCKING, self.db.get_history_path_by_index) as lock:
            ...
            new_id = self.removed_ids.add_oid_record((contributor, tile_list, "TBD", transaction_id, 1, 0))
            ...
            self.removed_ids[new_id] = (contributor, tile_list, ",".join([str(c) for c in contributor_tiles]), transaction_id, 1, 1)
    ...
    def reinsert_from_sqlite(self, comp_callback=None):
        ...
        while len(self.reinserts.unfinished_records()) > 0:
            ...
            time.sleep(2)
            for oid, contributor, limit_to_tiles, started, finished in self.reinserts.unfinished_records():  # if we use the generator it may have trouble when records are removed by other processes
                try:
                    fname, tiles, override_epsg, reverse_z, survey_score, flag, dformat, mtime, trans_id = self.included_ids[contributor]
                except KeyError:
                    try:
                        fname, tiles, override_epsg, reverse_z, survey_score, flag, dformat, mtime, trans_id = self.included_ids[str(contributor)]
                    except KeyError:
                        fname, tiles, override_epsg, reverse_z, survey_score, flag, dformat, mtime, trans_id = self.included_ids[int(contributor)]
                        # FIXME - if a survey is being re-inserted that never finished then it will not be found in included_ids.
                        #    Either make sure unfinished surveys had been removed or add the extra parameters to the started_ids data.
                try:
                    lock = FileLock(fname)  # this doesn't work with the file lock - just the multiprocessing locks
                    if lock.acquire():
                        contributor, limit_to_tiles, started, finished = self.reinserts[oid]
                        if not finished:  # still in the database, did another process remove it?
                            self.reinserts.set_started(oid)
                            self.insert_survey(fname, contrib_id=contributor, compare_callback=comp_callback, override_epsg=override_epsg,
                                               reverse_z=reverse_z,
                                               limit_to_tiles=limit_to_tiles, force=True, dformat=dformat, transaction_id=trans_id)
    ...
    def add_reinserts(self, contributors:dict):
        existing_unfinished = {i[1]: i for i in self.reinserts.unfinished_records()}

        for contrib, tiles in contributors.items():
            try:
                existing = existing_unfinished[contrib]
                updated = set(existing[2])  # [2] is the tile list  @todo change the sqlite records classes to allow names to be used like .tiles
                updated.update(tiles)
                self.reinserts[existing[0]] = tuple(updated)  # [0] is the oid for the record
            except KeyError:
