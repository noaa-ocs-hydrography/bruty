...
def create_identity_column(table_name, start_val, conn_info: ConnectionInfo, col_name=NBS_ID_STR, force_restart=False, drop_add=False):
    ...
    if ret is None:  # no column named 'col_name' in the table
        ...
        if is_ident.upper() == "NO":
            if drop_add:  # replace the column from scratch
                ...
                cursor.execute(f"ALTER TABLE {table_name} ADD CONSTRAINT {table_name}_nbs_id_unique UNIQUE ({col_name})")
                connection.commit()
        else:
    ...
    if needs_id_column:
        ...
        cursor.execute(f"ALTER TABLE {table_name} ADD COLUMN {col_name} bigint GENERATED BY DEFAULT AS IDENTITY UNIQUE")
        connection.commit()
        force_restart = True
